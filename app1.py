...
import git  # Use GitPython to avoid shell calls
...
sidebar_option = st.sidebar.radio("Choose an option:", ["Dashboard", "Analyze Repositories", "View Reports", "Score Details"])
...
def analyze_repo(repo_url):
    try:
        ...
        # Clone the repository using GitPython (avoids os.system)
        ...
        # Check for README.md file (optional)
        ...
        if os.path.exists(readme_path):
            ...
            readme_analysis = "README.md not available."

        # Limit the analysis to certain types of files (e.g., .py, .js)
        supported_files = ['.py', '.js', '.java']
        for root, _, files in os.walk(repo_path):
            for file in files:
                if any(file.endswith(ext) for ext in supported_files):  # Check for specific file types
        ...
        # Add readme_analysis to the report data but don't give it a score
...
if sidebar_option == "Dashboard":
    st.header("Welcome to the GitHub Repository Analyzer Dashboard")
    st.write("Here, you can analyze multiple GitHub repositories, view their scores, and download reports.")

elif sidebar_option == "Analyze Repositories":
...
elif sidebar_option == "View Reports":
    st.header("View Your Previous Reports")
    st.write("Currently, no saved reports are available. You may upload or download reports once you analyze repositories.")

elif sidebar_option == "Score Details":
    st.header("Score Details")
    st.write("This section will show detailed analysis, including scores for each file analyzed.")
